...
utt_num=301
lin_dir=exp/nnet3/tdnn_lstm1a_lin
decode_adapt_dir=$lin_dir/decode_adapt_hubei_lin_${utt_num}
...
# Insert LIN layer
if [ $stage -le -1 ]; then
  mkdir -p $lin_dir || exit 1;
  echo "======================================================================"
  echo "               Insert LIN Layer and Initialize                        "
  echo "======================================================================"

  nnet3-am-copy --binary=false $am_dir/final.mdl $lin_dir/si_am.mdl
  misc/insert_lin_layer.py --lda_dim=300 --nnet_in=$lin_dir/si_am.mdl \
    --nnet_out=$lin_dir/insert.mdl

  nnet3-am-copy --binary=true --edits-config=$edits_conf \
    $lin_dir/insert.mdl $lin_dir/lin.mdl
fi
...
if [ $stage -le 3 ]; then
  ...
  (
    ...
    dir=$lin_dir/lin_${train_speaker}_$utt_num
    ...
    steps/nnet3/get_egs.sh --cmd "$train_cmd" \
      ...
      --srand $srand \
      $train_data_dir $adapt_ali_dir $train_data_dir/egs_lin_${utt_num}
    ...
    cp $lin_dir/lin.mdl $dir/0.mdl || exit 1;
    ...
    steps/nnet3/train_rnn.py --stage=$train_stage \
      ...
      --egs.dir="$train_data_dir/egs_lin_${utt_num}" \
      --cleanup="false" \
...
if [ $stage -le 4 ]; then
  ...
  (
    ...
    # cp $lin_dir/lin_${data}_$utt_num/final.mdl $decode_dir/final.mdl || exit 1;
    cp $lin_dir/lin_${data}_$utt_num/1.mdl $decode_dir/final.mdl || exit 1;
...
acwt=0.15    # default is 0.1
decode_adapt_dir=$lin_dir/decode_adapt_hubei_lin_100_${utt_num}
# Decode
if [ $stage -le 6 ]; then
  echo "======================================================================"
  echo "                            Decoding                                  "
  ...
  (
    ...
    mkdir -p $decode_dir || exit 1;
    # cp $lin_dir/lin_${data}_$utt_num/final.mdl $decode_dir/final.mdl || exit 1;
    cp $lin_dir/lin_${data}_$utt_num/8.mdl $decode_dir/final.mdl || exit 1;
    cp $am_dir/cmvn_opts $decode_dir || exit 1;
    if [ -f $decode_adapt_dir/.error ]; then
      rm $decode_adapt_dir/.error || exit 1;
    fi
