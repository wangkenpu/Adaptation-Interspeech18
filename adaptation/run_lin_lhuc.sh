...
utt_num=21
lin_lhuc_dir=exp/nnet3/tdnn_lstm1a_lin_lhuc
decode_adapt_dir=$lin_lhuc_dir/decode_adapt_hubei_lin_lhuc_${utt_num}
...
# Insert LIN and LHUC layer
if [ $stage -le -1 ]; then
  mkdir -p $lin_lhuc_dir || exit 1;
  echo "======================================================================"
  echo "               Insert LIN and LHUC Layer and Initialize                        "
  echo "======================================================================"

  nnet3-am-copy --binary=false $am_dir/final.mdl $lin_lhuc_dir/si_am.mdl
  nnet3-am-copy --binary=false --edits-config=$edits_conf \
    $lin_lhuc_dir/si_am.mdl $lin_lhuc_dir/fixed_am.mdl

  # LIN layer
  misc/insert_lin_layer.py --lda_dim=300 --nnet_in=$lin_lhuc_dir/fixed_am.mdl \
    --nnet_out=$lin_lhuc_dir/lin.mdl
  # LHUC layer
  python misc/insert_lhuc_layer.py  --nnet-in=$lin_lhuc_dir/lin.mdl \
    --nnet-out=$lin_lhuc_dir/insert.mdl

  nnet3-am-copy --binary=true $lin_lhuc_dir/insert.mdl $lin_lhuc_dir/lin_lhuc.mdl
fi
...
if [ $stage -le 3 ]; then
  ...
  (
    ...
    dir=$lin_lhuc_dir/lin_lhuc_${train_speaker}_$utt_num
    ...
    steps/nnet3/get_egs.sh --cmd "$train_cmd" \
      ...
      --srand $srand \
      $train_data_dir $adapt_ali_dir $train_data_dir/egs_lin_lhuc_${utt_num}
    ...
    cp $lin_lhuc_dir/lin_lhuc.mdl $dir/0.mdl || exit 1;
    ...
    steps/nnet3/train_rnn.py --stage=$train_stage \
      ...
      --egs.dir="$train_data_dir/egs_lin_lhuc_${utt_num}" \
      --cleanup="false" \
      ...
      --dir=$dir  || exit 1;
    echo "LOG (LIN Adaptation): Do adaptation for $train_speaker sucessed."
  ) || touch exp/.error &
  done
  wait
...
if [ $stage -le 4 ]; then
  ...
  (
    ...
    # cp $lin_lhuc_dir/lin_lhuc_${data}_$utt_num/final.mdl $decode_dir/final.mdl || exit 1;
    cp $lin_lhuc_dir/lin_lhuc_${data}_$utt_num/3.mdl $decode_dir/final.mdl || exit 1;
    cp $am_dir/cmvn_opts $decode_dir || exit 1;
    if [ -f $decode_adapt_dir/.error ]; then
      rm $decode_adapt_dir/.error || exit 1;
    fi
    ...
    steps/nnet3/decode.sh \
      ...
      --nj $nj --cmd "$train_cmd"  --num-threads 1 \
  ...
  echo "LOG (LIN Decode): Compute CER sucessed."
fi
exit 0;
...
acwt=0.15    # default is 0.1
decode_adapt_dir=$lin_lhuc_dir/decode_adapt_hubei_lin_lhuc_100_${utt_num}
# Decode
if [ $stage -le 6 ]; then
  echo "======================================================================"
  echo "                            Decoding                                  "
  ...
  (
    ...
    mkdir -p $decode_dir || exit 1;
    # cp $lin_lhuc_dir/lin_lhuc_${data}_$utt_num/final.mdl $decode_dir/final.mdl || exit 1;
    cp $lin_lhuc_dir/lin_lhuc_${data}_$utt_num/7.mdl $decode_dir/final.mdl || exit 1;
    cp $am_dir/cmvn_opts $decode_dir || exit 1;
    if [ -f $decode_adapt_dir/.error ]; then
      rm $decode_adapt_dir/.error || exit 1;
    fi
