...
utt_num=31
kld_dir=exp/nnet3/tdnn_lstm1a_kld
decode_adapt_dir=$kld_dir/decode_adapt_hubei_kld_${utt_num}
...
if [ $stage -le 1 ]; then
  ...
  [ -f exp/nnet3${nnet3_affix}/ivectors_adapt_train/.error ] && \
    echo "$0: there was a problem while extracting train ivector" && exit 1;
fi
...
if [ $stage -le 3 ]; then
  ...
  (
    ...
    dir=$kld_dir/kld_${train_speaker}_${utt_num}
    ...
    steps/nnet3/get_egs.sh --cmd "$train_cmd" \
      ...
      --srand $srand \
      $train_data_dir $adapt_ali_dir $train_data_dir/egs_kld_${utt_num}
    ...
    cp $am_dir/final.mdl $dir/0.mdl || exit 1;
    ...
    steps_kld/nnet3/train_rnn.py --stage=$train_stage \
      ...
      --trainer.optimization.initial-effective-lrate=1e-3 \
      --trainer.optimization.final-effective-lrate=1e-4 \
      ...
      --egs.dir="$train_data_dir/egs_kld_${utt_num}" \
      --cleanup="false" \
      ...
      --dir=$dir  || exit 1;
    echo "LOG (KLD Adaptation): Do adaptation for $train_speaker sucessed."
  ) || touch exp/.error &
  done
  wait
...
if [ $stage -le 4 ]; then
  ...
  (
    ...
    if [ -d $decode_dir ]; then
      rm -rf $decode_dir || exit 1;
    fi
    mkdir -p $decode_dir || exit 1;
    # cp $kld_dir/kld_${data}_$utt_num/final.mdl $decode_dir/final.mdl || exit 1;
    cp $kld_dir/kld_${data}_$utt_num/10.mdl $decode_dir/final.mdl || exit 1;
    cp $kld_dir/kld_${data}_$utt_num/cmvn_opts $decode_dir || exit 1;
    ...
    steps/nnet3/decode.sh \
      ...
      --nj $nj --cmd "$score_cmd"  --num-threads 1 \
    ...
    steps/scoring/score_kaldi_cer.sh --cmd "$score_cmd" \
  ...
  echo "LOG (KLD Decode): Compute CER sucessed."
fi
exit 0;
...
acwt=0.15    # default is 0.1
decode_adapt_dir=$kld_dir/decode_adapt_hubei_kld_100_${utt_num}
# Decode
if [ $stage -le 6 ]; then
  echo "======================================================================"
  echo "                            Decoding                                  "
  ...
  (
    ...
    mkdir -p $decode_dir || exit 1;
    # cp $kld_dir/kld_${data}_$utt_num/final.mdl $decode_dir/final.mdl || exit 1;
    cp $kld_dir/kld_${data}_$utt_num/2.mdl $decode_dir/final.mdl || exit 1;
    cp $kld_dir/kld_${data}_$utt_num/cmvn_opts $decode_dir || exit 1;
    if [ -f $decode_adapt_dir/.error ]; then
      rm $decode_adapt_dir/.error || exit 1;
    fi
