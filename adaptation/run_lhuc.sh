...
utt_num=301
lhuc_dir=exp/nnet3/tdnn_lstm1a_lhuc
decode_adapt_dir=$lhuc_dir/decode_adapt_hubei_lhuc_${utt_num}
...
# Insert LHUC layer
if [ $stage -le -1 ]; then
  mkdir -p $lhuc_dir || exit 1;
  echo "======================================================================"
  echo "               Insert LHUC Layer and Initialize                        "
  echo "======================================================================"

  nnet3-am-copy --binary=false  $am_dir/final.mdl $lhuc_dir/si_am.mdl
  nnet3-am-copy --binary=false --edits-config=$edits_conf \
    $lhuc_dir/si_am.mdl $lhuc_dir/fixed_am.mdl
  python misc/insert_lhuc_layer.py  --nnet-in=$lhuc_dir/fixed_am.mdl \
    --nnet-out=$lhuc_dir/insert.mdl
  nnet3-am-copy --binary=true $lhuc_dir/insert.mdl $lhuc_dir/lhuc.mdl
fi
...
if [ $stage -le 3 ]; then
  ...
  (
    ...
    dir=$lhuc_dir/lhuc_${train_speaker}_$utt_num
    ...
    steps/nnet3/get_egs.sh --cmd "$train_cmd" \
      ...
      --srand $srand \
      $train_data_dir $adapt_ali_dir $train_data_dir/egs_lhuc_${utt_num}
    ...
    cp $lhuc_dir/lhuc.mdl $dir/0.mdl || exit 1;
    ...
    steps/nnet3/train_rnn.py --stage=$train_stage \
      ...
      --egs.dir="$train_data_dir/egs_lhuc_${utt_num}" \
      --cleanup="false" \
...
if [ $stage -le 4 ]; then
  ...
  (
    ...
    # cp $lhuc_dir/lhuc_${data}_$utt_num/final.mdl $decode_dir/final.mdl || exit 1;
    cp $lhuc_dir/lhuc_${data}_$utt_num/1.mdl $decode_dir/final.mdl || exit 1;
  ...
  echo "LOG (LHUC Decode): Compute CER sucessed."
fi
#exit 0;
...
acwt=0.15    # default is 0.1
decode_adapt_dir=$lhuc_dir/decode_adapt_hubei_lhuc_100_${utt_num}
# Decode
if [ $stage -le 6 ]; then
  echo "======================================================================"
  echo "                            Decoding                                  "
  ...
  (
    ...
    mkdir -p $decode_dir || exit 1;
    # cp $lhuc_dir/lhuc_${data}_$utt_num/final.mdl $decode_dir/final.mdl || exit 1;
    cp $lhuc_dir/lhuc_${data}_$utt_num/7.mdl $decode_dir/final.mdl || exit 1;
    cp $am_dir/cmvn_opts $decode_dir || exit 1;
    if [ -f $decode_adapt_dir/.error ]; then
      rm -f $decode_adapt_dir/.error || exit 1;
    fi
