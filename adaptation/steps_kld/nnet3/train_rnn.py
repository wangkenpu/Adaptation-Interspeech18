...
def get_args():
    ...
    parser.add_argument("--trainer.kld-rho", type=float,
                        dest='kld_rho', default=0.0625,
                        help="""KLD regularization weight for adaptation
                        training. When this parameter is set to be 0, we
                        adapt the model solely on the adapt set. Suggested
                        range is [0.0625, 0.5].""")
    parser.add_argument("--trainer.si-model", type=str,
                        dest='si_model', default=None,
                        help="""SI model for KLD adaptation training.""")
...
def train(args, run_opts):
    ...
    logger.info("Training will run for {0} epochs = "
                "{1} iterations, KLD weight is {2}".format(
                    args.num_epochs, num_iters, args.kld_rho))
    ...
    for iter in range(num_iters):
        ...
        if args.stage <= iter:
            ...
            train_lib.common.train_one_iteration(
                ...
                kld_rho=args.kld_rho, si_model=args.si_model)
